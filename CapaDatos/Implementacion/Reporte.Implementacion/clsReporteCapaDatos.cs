...
namespace CapaDatos.Implementacion.Reporte.Implementacion
{
    public class clsReporteCapaDatos: IReporteCapaDatos
    {
...

        {
            ...
            if (dtInformacion.Rows.Count > 0)
            {
                for (int rows = 0; rows < dtInformacion.Rows.Count; rows++)
                {
                    ...
                    objeto.CategoriaTercero =dtInformacion.Rows[rows]["CategoriaTercero"].ToString();                     objeto.NombreRazonSocial = dtInformacion.Rows[rows]["NombreRazonSocial"].ToString();
...

        {
            ...
            Consulta = string.Format("SELECT a.[Id] ,a.[IdFormulario],a.[ObligadoProgramaLA],a.[ObligadoProgramaEE],a.[TieneContratosSP],a.[ActividadesActivosVirtuales],a.[IntercambiosActivosMoneda],a.[IntercambioActivosVirtuales],a.[TransferenciasActivosVirtuales],a.[CustodiaAdministraAC],a.[ParticipacionSFAV],a.[ServiciosAV],a.[IngresosYearAnterior] FROM [dbo].[CumplimientoNormativo] as a inner join [dbo].[FormularioClienteProveedores] fcp on (a.IdFormulario=fcp.Id) {0}", where);
            DataTable dtInformacion = new DataTable();
            try
            {
                cDataBase.conectar();
            ...
            if (dtInformacion.Rows.Count > 0)
            {
                for (int rows = 0; rows < dtInformacion.Rows.Count; rows++)
                {
                    ...
                    objeto.Id = Convert.ToInt32(dtInformacion.Rows[rows]["Id"]);
                    objeto.IdFormulario = Convert.ToInt32(dtInformacion.Rows[rows]["IdFormulario"]);
                    objeto.ObligadoProgramaLA = Convert.ToInt32(dtInformacion.Rows[rows]["ObligadoProgramaLA"]);
                    objeto.ObligadoProgramaEE = Convert.ToInt32(dtInformacion.Rows[rows]["ObligadoProgramaEE"]);
                    objeto.TieneContratosSP = Convert.ToInt32(dtInformacion.Rows[rows]["TieneContratosSP"]);
                    objeto.ActividadesActivosVirtuales = Convert.ToInt32(dtInformacion.Rows[rows]["ActividadesActivosVirtuales"]);


                    objeto.IntercambiosActivosMoneda = Convert.ToInt32(dtInformacion.Rows[rows]["IntercambiosActivosMoneda"]);
                    objeto.IntercambioActivosVirtuales = Convert.ToInt32(dtInformacion.Rows[rows]["IntercambioActivosVirtuales"]);
                    objeto.TransferenciasActivosVirtuales = Convert.ToInt32(dtInformacion.Rows[rows]["TransferenciasActivosVirtuales"]);
                    objeto.CustodiaAdministraAC = Convert.ToInt32(dtInformacion.Rows[rows]["CustodiaAdministraAC"]);
                    objeto.ParticipacionSFAV = Convert.ToInt32(dtInformacion.Rows[rows]["ParticipacionSFAV"]);
                    objeto.ServiciosAV = Convert.ToInt32(dtInformacion.Rows[rows]["ServiciosAV"]);
                    objeto.IngresosYearAnterior = Convert.ToInt32(dtInformacion.Rows[rows]["IngresosYearAnterior"]);
        ...
        public async Task<List<FormularioModelDTO>> ConsultaDatosInformacionOEA(ReporteDto objetofiltro)
        {
            ...
            if (dtInformacion.Rows.Count > 0)
            {
                for (int rows = 0; rows < dtInformacion.Rows.Count; rows++)
                {
                    ...
                    objeto.NumeroDias= dtInformacion.Rows[rows]["NumeroDias"].ToString(); 
