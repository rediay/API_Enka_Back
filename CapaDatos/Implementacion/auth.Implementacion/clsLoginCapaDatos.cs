    ...
    public class clsLoginCapaDatos : IloginCapaDatos
    {
        ...
        public async Task<bool> CrearUsuario(UsuarioDto objRegistro)
        {
            ...
            try
            {
                ...
                int scope = 0;
                strConsulta = string.Format("insert into [dbo].[tbl_Usuarios] ([Nombre],[Apellidos],[Email],[Password],[TipoUsuario], "+
                    "[FechaCreacion],[Identificacion],[Activo],[ActualizarPass],[Usuario],[IdCompradorVendedor]) values ( " +
                    "'{0}','{1}','{2}','{3}',{4},getdate(),'{5}',1,1,'{5}',{6}) SELECT SCOPE_IDENTITY()",
                    objRegistro.Nombres,objRegistro.Apellidos, objRegistro.Email, strTemporalPASS,objRegistro.IdTipoUsuario,
                    Usuario, objRegistro.idCompradorVendedor);
